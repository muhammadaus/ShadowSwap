...
describe('AMM', () => {
  ...
  describe('Swapping tokens', () => {
    ...
    it('facilitates swaps', async () => {
      ...
      /////////////////////////////////////////////////////////////
      // Set up swap approvals
      //

      // Investor1 approves deployer to swap on their behalf
      console.log('before swap')
      transaction = await amm.connect(investor1).setSwapApproval(deployer.address, true)
      await transaction.wait()

      // Investor2 approves deployer to swap on their behalf
      transaction = await amm.connect(investor2).setSwapApproval(deployer.address, true)
      await transaction.wait()

      /////////////////////////////////////////////////////////////
      // Investor 3 swaps on behalf of Investor 1
      //
      ...
      // deployer swaps 1 token1 on behalf of Investor1
      transaction = await amm.connect(deployer).swapToken1(tokens(1), investor1.address)
      ...
      /////////////////////////////////////////////////////////////
      // Investor 3 swaps on behalf of Investor 2
      //
      ...
      // deployer swaps 1 token2 on behalf of Investor2
      transaction = await amm.connect(deployer).swapToken2(tokens(1), investor2.address)
      await transaction.wait()
    ...
    it('fails when swap is not approved', async () => {
      // deployer tries to swap on behalf of Investor1 without approval
      await expect(
        amm.connect(deployer).swapToken1(tokens(1), investor1.address)
      ).to.be.revertedWith("Not authorized to swap on behalf of owner")

      // Investor1 revokes approval for deployer
      await amm.connect(investor1).setSwapApproval(deployer.address, false)

      // deployer tries to swap on behalf of Investor1 after approval is revoked
      await expect(
        amm.connect(deployer).swapToken1(tokens(1), investor1.address)
      ).to.be.revertedWith("Not authorized to swap on behalf of owner")
    })
