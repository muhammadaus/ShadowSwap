...
const Swap = () => {
  ...
  useEffect(() => {
    const loadTokensData = async () => {
      try {
        await loadTokens(provider, [tokens[0].address, tokens[1].address], dispatch)
        setTokenLoadError(null)
      } catch (error) {
        console.error("Error loading tokens:", error)
        setTokenLoadError(`Error loading tokens: ${error.message}. Please check token addresses.`)
      }
    }
